...
<?php
  ...
  {
?>
<h1>MeerKAT Cam Simulator - Observing Parameters</h1>
...
<?php
  ...
  function printSPIPResponse($get)
  {
    ...
    $pubsub_host = $this->config["SERVER_HOST"];
    $pubsub_port = $this->config["MEERKAT_PUBSUB_PORT"];
    ...
    $pubsub_socket = new spip_socket();
    if ($pubsub_socket->open ($pubsub_host, $pubsub_port) == 0)
    {
      $raw_xml = str_replace("\n","", $beam_xml);
      $pubsub_socket->write ($raw_xml."\r\n");
      $reply = $pubsub_socket->read();
    }
    else
    {
     $html .= "<p>Could not connect to ".$pubsub_host.":".$pubsub_port."</p>\n";
    }
    $pubsub_socket->close();
