...
int main(int argc, char *argv[]) try
{
  ...
  if (verbose)
    cerr << "caspsr_dbstats: creating DataBlockStats with " << key << endl;
  dbstats = new spip::DataBlockStats (key.c_str());

  dbstats->set_verbosity(verbose > 0);
  dbstats->set_block_format (new spip::BlockFormatCASPSR());
  ...
  if (verbose)
    cerr << "caspsr_dbstats: reading config from " << config_file << endl;
  ...
  if (verbose)
    cerr << "caspsr_dbstats: configuring using fixed config" << endl;
  ...
  if (control_port > 0)
  {
    cerr << "caspsr_dbstats: start_control_thread (" << control_port << ")" << endl;
    ...
    {
      if (verbose)
        cerr << "caspsr_dbstats: monitor()" << endl;
      keep_monitoring = dbstats->monitor(stats_dir, stream);
      if (verbose)
        cerr << "caspsr_dbstats: monitor returned" << endl;
    }
  }
  else
  {
    cerr << "caspsr_dbstats: calling monitor" << endl;
...
void usage() 
{
  cout << "caspsr_dbstats [options] config\n"
